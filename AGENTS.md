...
- Do not open a pull request until both `npm run test` and `npm run build` have completed successfully locally. If either validation fails, fix the issue before creating the pull request. Do NOT weaken assertions just to make tests pass.
...
- Never manually change the `package-lock.json` file. Any changes to this file should be automatically applied as an effect of running install commands with `npm` (such as `npm i`, `npm ci`).
...
- You can run `npm` install commands freely without asking for my permission. This includes any command starting with `npm i`, `npm ci`, or `npm install`. 
- Use Node version specified in `.nvmrc`.
- Prefer `nvm use` without specifying a version.